...
def set_value_setter_store(interval, switch, data):
    if not interval:
        ...
        # 如果是 "switch-select-dropdown", "value" 变化了就清空 "value-setter-store", "data"
        if trigger_id == "switch-select-dropdown":
            return {}, port_data
        
    
        # 假设这是一次新的数据更新
        new_data = [[4294967294, 0, 0, 2107.899], [1, 16072, 16072, 1429.083], [2, 16030, 16030, 1425.245], [3, 15554, 15554, 1425.244]]
        
        # 将新数据转换为DataFrame
        df_new = pd.DataFrame(new_data, columns=['port', 'rx_bytes', 'tx_bytes', 'duration'])
        
        # 通过groupby和concat更新port_dir字典
        for port, group in df_new.groupby('port'):
            if port in port_dir:
                # 如果已经有这个端口的数据，则用concat合并
                port_dir[port] = {
                    'rx_bytes': pd.concat([pd.Series(port_dir[port]['rx_bytes']), group['rx_bytes']]).tolist(),
                    'tx_bytes': pd.concat([pd.Series(port_dir[port]['tx_bytes']), group['tx_bytes']]).tolist(),
                    'duration': pd.concat([pd.Series(port_dir[port]['duration']), group['duration']]).tolist(),
                }
            else:
                # 如果这个端口的数据还不存在，则进行初始化
                port_dir[port] = {
                    'rx_bytes': group['rx_bytes'].tolist(),
                    'tx_bytes': group['tx_bytes'].tolist(),
...
#             interval, port, stored_data
#         )
#         spark_line_data = update_sparkline(port, rate, data)
#         return port, time, spark_line_data, rate, occ, color
#
#     return callback
#
#
...
#     inputs=Input('interval-component', 'n_intervals'),
#     state=State('port-store', 'data'),
# )
# def dynamic_callback_creation(interval, stored_data):
#
...
#
#     return "Dynamic callbacks created"
...
def update_control_chart(interval, data, figure, clicks):
    ...
    # if triggered_value == 'n_intervals':
    #     port = figure["data"][0]["name"]
    #     if port == "":
    #         port = next(iter(data.keys()))
    #     # print("n_intervals:{}".format(port))
    #     return generate_graph(interval, data, port)
    # elif  triggered_value == 'n_clicks':
    #     triggered_id = json.loads(triggered_id)
    #     port = triggered_id["index"]
    #     # print("n_clicks:{}".format(port))
    #     return generate_graph(interval, data, port)
